...
parseSCPsection scprec remains crc
  ...
  | remains < 8  = return $! (Left ("short data " ++ (show remains)), 0, crc)
  | otherwise = do
    seccrc <- getWord16le
    sec_id <- getWord16le
    seclen <- getWord32le
    skip ((fromIntegral seclen) - 8)
    let newrec = SCPRec {sec11 = Just ((fromIntegral sec_id):(fromIntegral seclen):(fromMaybe [] (sec11 scprec)))}
...
parseSCPfile realsize = do
  ...
  if realsize == (fromIntegral expectedsize) then do
      (scprec, _, realcrc) <- parseSCPsection emptySCPRec (realsize - 6) 0
